...
const PhotoUploader = ({addedPhotos,onChange}) => {
  ...
  e.preventDefault();
  console.log('Adding photo by link:', photoLink);

  try {
    const { data: filename } = await axios.post('/upload-by-link', { link: photoLink });
    console.log('Response:', filename);
    onChange((prev) => [...prev, filename]);
    console.log('Updated addedPhotos:', addedPhotos);
    setPhotoLink('');
  } catch (error) {
    ...
    function selectMainCover(e,filename){
        e.preventDefault()
        const addedPhotosWitoutSelected = addedPhotos.filter(photo => photo !== filename)
        const newAddedPhotos = [filename,...addedPhotosWitoutSelected]
                    ...
                    <input type='file' className='hidden' />
                       ...
                       <div className='mt-2 grid grid-cols-3 md:grid:cols-4 lg:grid-cols-6'>
                            {addedPhotos.length > 0 && addedPhotos.map((link,index) => {
                                return (
                                    <div key={index} className='h-32 relative flex mx-2'>
                                        
                                       <img className='rounded-2xl w-full object-cover position-center gap-2' src={'http://localhost:4000/uploads/'+link}/>
                                    <button  onClick={(e)=> removePhoto(e,link)} className="cursor-pointer absolute bottom-1 right-1 text-white bg-black bg-opacity-50 rounded-2xl py-2 px-3">
                                        ...
                                        <button  onClick={(e)=> selectMainCover(e,link)} className="cursor-pointer absolute bottom-1 left-1 text-white bg-black bg-opacity-50 rounded-2xl py-2 px-3">
                                           {link === addedPhotos[0] && (
